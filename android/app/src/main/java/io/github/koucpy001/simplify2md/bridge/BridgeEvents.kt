 ...
 *
 * These three strings are the contract with the frontend: `App.vue` registers a
 * listener for each one with `EventsOn` (confirm-exit, open-path, file-changed),
 * and the Android shim `window.__bridgeEmit(name, payloadJson)` dispatches to
...
object BridgeEvents {
    ...
    val REGISTERED: Set<String> = setOf(CONFIRM_EXIT, OPEN_PATH, FILE_CHANGED)
